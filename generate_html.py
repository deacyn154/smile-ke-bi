...
function barPct(v, t, timePct) { if(!t)return'';let p=v/t*100;if(timePct!==undefined&&p<timePct)return'<td class=\"bar-cell\" style=\"position:relative\"><div class=\"bar\" style=\"width:'+Math.min(p,100)+'%;background:var(--red)\"></div><span style=\"color:var(--red);font-weight:600\">'+p.toFixed(1)+'%</span></td>';let c=p>=45?'progress-good':p>=30?'progress-warn':'progress-bad';return'<td class=\"bar-cell\" style=\"position:relative\"><div class=\"bar\" style=\"width:'+Math.min(p,100)+'%;background:var(--'+c.replace('progress-','')+')\"></div><span class=\"'+c+'\">'+p.toFixed(1)+'%</span></td>'; }
